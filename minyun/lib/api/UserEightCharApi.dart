...
class UserEightCharApi{
  ...
  static Future<UserEightCharModel> getInfo(int id) async {
    ...
    if (jsonMap['code'] == 200) {
      ...
      return UserEightCharModel.fromJson(jsonMap['data']);
    } else {
  ...
  static Future<ResultListModel<UserEightCharModel>> getList(Map<String, String> queryParams) async {
    final jsonMap = await HttpUtil.get(list, queryParams);

    ResultListModel<UserEightCharModel> result = ResultListModel.fromJson(jsonMap, (json) => UserEightCharModel.fromJson(json));
    if (result.code == 200) {
      return result;
    } else {
  ...
  static Future<List<UserEightCharModel>> _performNetworkRequest(String url, Map<String, String> queryParams, String prefsKey) async {
    final jsonMap = await HttpUtil.get(url, queryParams);

    if (jsonMap['code'] == 200) {
      List<dynamic> jsonData = jsonMap['data'];
      List<UserEightCharModel> values = jsonData.map<UserEightCharModel>((item) => UserEightCharModel.fromJson(item as Map<String, dynamic>)).toList();
  ...
  static List<UserEightCharModel> _parseAlbumModelList(String jsonData) {
    List<dynamic> listData = json.decode(jsonData);
    return listData.map((item) => UserEightCharModel.fromJson(item as Map<String, dynamic>)).toList();
  }
