...
import 'package:minyun/models/chat_content_model.dart';
...
class ChatContentApi{
  ...
  static Future<ChatContentModel> getInfo(int id) async {
    ...
    if (jsonMap['code'] == 200) {
      ...
      return ChatContentModel.fromJson(jsonMap['data']);
    } else {
  ...
  static Future<ResultListModel<ChatContentModel>> getList(Map<String, String> queryParams) async {
    final jsonMap = await HttpUtil.get(list, queryParams);

    ResultListModel<ChatContentModel> result = ResultListModel.fromJson(jsonMap, (json) => ChatContentModel.fromJson(json));
    if (result.code == 200) {
      return result;
    } else {
  ...
  static Future<List<ChatContentModel>> _performNetworkRequest(String url, Map<String, String> queryParams, String prefsKey) async {
    final jsonMap = await HttpUtil.get(url, queryParams);

    if (jsonMap['code'] == 200) {
      List<dynamic> jsonData = jsonMap['data'];
      List<ChatContentModel> values = jsonData.map<ChatContentModel>((item) => ChatContentModel.fromJson(item as Map<String, dynamic>)).toList();
  ...
  static List<ChatContentModel> _parseAlbumModelList(String jsonData) {
    List<dynamic> listData = json.decode(jsonData);
    return listData.map((item) => ChatContentModel.fromJson(item as Map<String, dynamic>)).toList();
  }
