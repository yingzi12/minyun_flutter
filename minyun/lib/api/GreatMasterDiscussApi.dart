...
import 'package:minyun/models/great_master_discuss_model.dart';
...
class GreatMasterDiscussApi{
  ...
  static Future<GreatMasterDiscussModel> getInfo(int id) async {
    ...
    if (jsonMap['code'] == 200) {
      ...
      return GreatMasterDiscussModel.fromJson(jsonMap['data']);
    } else {
  ...
  static Future<ResultListModel<GreatMasterDiscussModel>> getList(Map<String, String> queryParams) async {
    final jsonMap = await HttpUtil.get(list, queryParams);

    ResultListModel<GreatMasterDiscussModel> result = ResultListModel.fromJson(jsonMap, (json) => GreatMasterDiscussModel.fromJson(json));
    if (result.code == 200) {
      return result;
    } else {
  ...
  static Future<List<GreatMasterDiscussModel>> _performNetworkRequest(String url, Map<String, String> queryParams, String prefsKey) async {
    final jsonMap = await HttpUtil.get(url, queryParams);

    if (jsonMap['code'] == 200) {
      List<dynamic> jsonData = jsonMap['data'];
      List<GreatMasterDiscussModel> values = jsonData.map<GreatMasterDiscussModel>((item) => GreatMasterDiscussModel.fromJson(item as Map<String, dynamic>)).toList();
  ...
  static List<GreatMasterDiscussModel> _parseAlbumModelList(String jsonData) {
    List<dynamic> listData = json.decode(jsonData);
    return listData.map((item) => GreatMasterDiscussModel.fromJson(item as Map<String, dynamic>)).toList();
  }
