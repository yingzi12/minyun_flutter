...
class DiscussApi{
  ...
  static Future<DiscussModel> getInfo(int id) async {
    ...
    if (jsonMap['code'] == 200) {
      ...
      return DiscussModel.fromJson(jsonMap['data']);
    } else {
  ...
  static Future<ResultListModel<DiscussModel>> getList(Map<String, String> queryParams) async {
    final jsonMap = await HttpUtil.get(list, queryParams);

    ResultListModel<DiscussModel> result = ResultListModel.fromJson(jsonMap, (json) => DiscussModel.fromJson(json));
    if (result.code == 200) {
      return result;
    } else {
  ...
  static Future<List<DiscussModel>> _performNetworkRequest(String url, Map<String, String> queryParams, String prefsKey) async {
    final jsonMap = await HttpUtil.get(url, queryParams);

    if (jsonMap['code'] == 200) {
      List<dynamic> jsonData = jsonMap['data'];
      List<DiscussModel> values = jsonData.map<DiscussModel>((item) => DiscussModel.fromJson(item as Map<String, dynamic>)).toList();
  ...
  static List<DiscussModel> _parseAlbumModelList(String jsonData) {
    List<dynamic> listData = json.decode(jsonData);
    return listData.map((item) => DiscussModel.fromJson(item as Map<String, dynamic>)).toList();
  }
