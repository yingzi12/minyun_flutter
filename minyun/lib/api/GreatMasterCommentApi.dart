...
class GreatMasterCommentApi{
  ...
  static Future<GreatMasterCommentModel> getInfo(int id) async {
    ...
    if (jsonMap['code'] == 200) {
      ...
      return GreatMasterCommentModel.fromJson(jsonMap['data']);
    } else {
  ...
  static Future<ResultListModel<GreatMasterCommentModel>> getList(Map<String, String> queryParams) async {
    final jsonMap = await HttpUtil.get(list, queryParams);

    ResultListModel<GreatMasterCommentModel> result = ResultListModel.fromJson(jsonMap, (json) => GreatMasterCommentModel.fromJson(json));
    if (result.code == 200) {
      return result;
    } else {
  ...
  static Future<List<GreatMasterCommentModel>> _performNetworkRequest(String url, Map<String, String> queryParams, String prefsKey) async {
    final jsonMap = await HttpUtil.get(url, queryParams);

    if (jsonMap['code'] == 200) {
      List<dynamic> jsonData = jsonMap['data'];
      List<GreatMasterCommentModel> values = jsonData.map<GreatMasterCommentModel>((item) => GreatMasterCommentModel.fromJson(item as Map<String, dynamic>)).toList();
  ...
  static List<GreatMasterCommentModel> _parseAlbumModelList(String jsonData) {
    List<dynamic> listData = json.decode(jsonData);
    return listData.map((item) => GreatMasterCommentModel.fromJson(item as Map<String, dynamic>)).toList();
  }
