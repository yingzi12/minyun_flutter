...
import 'package:minyun/models/chat_partner_model.dart';
...
class ChatPartnerApi{
  ...
  static Future<ChatPartnerModel> getInfo(int id) async {
    ...
    if (jsonMap['code'] == 200) {
      ...
      return ChatPartnerModel.fromJson(jsonMap['data']);
    } else {
  ...
  static Future<ResultListModel<ChatPartnerModel>> getList(Map<String, String> queryParams) async {
    final jsonMap = await HttpUtil.get(list, queryParams);

    ResultListModel<ChatPartnerModel> result = ResultListModel.fromJson(jsonMap, (json) => ChatPartnerModel.fromJson(json));
    if (result.code == 200) {
      return result;
    } else {
  ...
  static Future<List<ChatPartnerModel>> _performNetworkRequest(String url, Map<String, String> queryParams, String prefsKey) async {
    final jsonMap = await HttpUtil.get(url, queryParams);

    if (jsonMap['code'] == 200) {
      List<dynamic> jsonData = jsonMap['data'];
      List<ChatPartnerModel> values = jsonData.map<ChatPartnerModel>((item) => ChatPartnerModel.fromJson(item as Map<String, dynamic>)).toList();
  ...
  static List<ChatPartnerModel> _parseAlbumModelList(String jsonData) {
    List<dynamic> listData = json.decode(jsonData);
    return listData.map((item) => ChatPartnerModel.fromJson(item as Map<String, dynamic>)).toList();
  }
