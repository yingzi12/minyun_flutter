...
import com.xinshijie.gallery.common.gif.AnimatedGifEncoder;
import com.xinshijie.gallery.common.vedio.ImageMetaInfo;
import com.xinshijie.gallery.common.vedio.MusicMetaInfo;
...
public class MediaUtil {
     ...
     * @param audioFile 从源视频提取的音频文件
     */
    public static void getAudioFromVideoToMp3(File videoFile, File audioFile) {
        if (null == videoFile || !videoFile.exists()) {
            throw new RuntimeException("源视频文件不存在： ");
        }
        if (null == audioFile) {
    ...
    /**
     * 将MP4视频切割为每10秒一个TS片段，并生成索引文件
     *
     * @param mp4File      源MP4视频文件
     * @param tsDirectory  存放TS文件的目录
     * @param playlistFile 生成的索引文件（例如M3U8播放列表）
     */
    public static void splitMp4ToTsSegmentsWithIndex(File mp4File, File tsDirectory, File playlistFile) {
        if (mp4File == null || !mp4File.exists()) {
            throw new RuntimeException("MP4源文件不存在");
        }
        if (tsDirectory == null || !tsDirectory.exists()) {
            throw new RuntimeException("TS文件目录不存在");
        }
        if (playlistFile == null) {
            throw new RuntimeException("索引文件路径为空");
        }
     ...
     * @param playlistFile
     */
    public static void splitVideoToTsSegmentsWithIndex(File videoFile, File tsDirectory, File playlistFile) {
        if (videoFile == null || !videoFile.exists()) {
            throw new RuntimeException("源视频文件不存在");
        }
        if (tsDirectory == null || !tsDirectory.exists()) {
        ...
        try {
            ...
            log.error("--- 切割视频到TS片段并生成索引文件过程中出现错误 ---", e);
        }
    }
